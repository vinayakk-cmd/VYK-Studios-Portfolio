...
function initSmoothScroll() {
    document.querySelectorAll('a[href^="#"]').forEach(a => {
        a.addEventListener('click', e => {
            ...
            window.scrollTo({
                top: Math.max(0, top), behavior: 'smooth'
            });
        });
    });
}
...
function initTestimonialsMarquee() {
    ...
    const baseDuration = cardCount * 5.5;
...
function initVideoGrid() {
    ...
    function fadeVol(video, targetVol, duration, onDone) {
        if (video._fadeRaf) {
            cancelAnimationFrame(video._fadeRaf);
            video._fadeRaf = null;
        }
    ...
    document.addEventListener('touchstart', e => {
        ...
        if (!e.target.closest || !e.target.closest('.vcard')) {
            touchActiveVideo = null; restoreAll();
        }
    }
...
/* ── Feedback: 3D Stacked Review Slider ── */
function initTestimonialsMarquee() {
    ...
    function updateStack() {
        ...
        cards.forEach((card, index) => {
            ...
            // Circular indexing to find shortest path
            ...
            if (rel === 0) {
                ...
                // 1st Card Right Behind
                const translateX = mobile ? '-44%' : '-42%';
                const rot = mobile ? '3deg' : '4.5deg';
                card.style.transform = `translate(${translateX}, 14px) scale(0.95) rotate(${rot})`;
                card.style.opacity = '0.75';
                card.style.zIndex = '8';
                card.style.pointerEvents = 'auto';
            } else if (rel === -1) {
                // 1st Card Left Behind
                const translateX = mobile ? '-56%' : '-58%';
                const rot = mobile ? '-3deg' : '-4.5deg';
                card.style.transform = `translate(${translateX}, 14px) scale(0.95) rotate(${rot})`;
                card.style.opacity = '0.75';
                card.style.zIndex = '8';
                ...
                // 2nd Card Right Behind
                const translateX = mobile ? '-38%' : '-34%';
                const rot = mobile ? '5deg' : '8deg';
                card.style.transform = `translate(${translateX}, 26px) scale(0.88) rotate(${rot})`;
                card.style.opacity = '0.4';
                card.style.zIndex = '6';
                card.style.pointerEvents = 'auto';
            } else if (rel === -2) {
                // 2nd Card Left Behind
                const translateX = mobile ? '-62%' : '-66%';
                const rot = mobile ? '-5deg' : '-8deg';
                card.style.transform = `translate(${translateX}, 26px) scale(0.88) rotate(${rot})`;
                card.style.opacity = '0.4';
                card.style.zIndex = '6';
                ...
                card.style.transform = 'translate(-50%, 38px) scale(0.8) rotate(0deg)';
...
function initServiceTilt() {
    document.querySelectorAll('.service-card').forEach(card => {
        ...
        card.addEventListener('mousemove', e => {
            ...
            card.style.transform = `perspective(1000px) translateY(-8px) rotateX(${-dy * 7}deg) rotateY(${-dx * 7}deg)`;
        });
...
function initAmbientBg() {
    ...
    if (!mobile) {
        window.addEventListener('mousemove', e => {
            ...
            orbs.forEach((o, i) => {
                targetX[i] = mx * o.fx; targetY[i] = my * o.fy;
            });
        }
